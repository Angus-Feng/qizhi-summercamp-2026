/**
 * 启智归塾2026夏令营 - 报名后端（EdgeOne Pages Node Functions 版）
 *
 * 部署位置：node-functions/express/[[default]].js
 * EdgeOne Pages 自动将请求路由到导出的 Express 实例
 *
 * API：
...
// ── 数据存储 ─────────────────────────────────────────────
const DATA_FILE = process.env.DATA_FILE || '/tmp/registrations.json';
...
function validate(data) {
  const errors = [];
  if (!data.parent_name || !data.parent_name.trim()) errors.push('家长姓名不能为空');
  if (!data.phone || !isValidPhone(data.phone)) errors.push('请输入正确的11位手机号');
  if (!data.wechat || !data.wechat.trim()) errors.push('微信号不能为空');
  if (!data.child_name || !data.child_name.trim()) errors.push('孩子姓名不能为空');
  if (!data.child_gender || !['男', '女'].includes(data.child_gender)) errors.push('请选择孩子性别');
  if (data.child_age == null || data.child_age < 6 || data.child_age > 18) errors.push('孩子年龄需在6-18岁之间');
  if (!data.product || !['7', '14', '21'].includes(data.product)) errors.push('请选择报名产品');
  if (!data.parent_accompany || !['yes', 'no'].includes(data.parent_accompany)) errors.push('请选择是否家长陪同');
  ...
  if (data.total_price === undefined || data.total_price < 0) errors.push('价格计算异常，请刷新页面后重试');
  if (!data.qa1 || !data.qa1.trim()) errors.push('请填写：孩子最近一次让你意外或触动的事');
  if (!data.qa2 || !data.qa2.trim()) errors.push('请填写：对本次夏令营的收获期待');
  return errors;
}
...
app.post('/api/register', (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, errors: ['服务器内部错误，请稍后重试'] });
  }
});
...
app.get('/api/registrations', (req, res) => {
  try {
    ...
    res.json({ success: true, data: rows, pagination: { page, page_size: pageSize, total, total_pages: Math.ceil(total / pageSize) } });
  } catch (err) {
...
app.get('/api/export', (req, res) => {
  try {
    ...
    const headers = ['ID','家长姓名','手机号','微信号','孩子姓名','孩子性别','孩子年龄','报名产品','家长陪同','陪同天数','孩子年级','特殊需求','特殊需求说明','推荐人','渠道来源','问答题1','问答题2','备注','早鸟优惠','团报优惠','老学员优惠','直播下单优惠','原价','优惠金额','陪同费','总价','报名时间'];
    const productLabels = { '7': '体验道场(7天)', '14': '进阶道场(14天)', '21': '完整道场(21天)' };

    const csvLines = [headers.map(esc).join(',')];
    for (const row of records) {
      csvLines.push([
        row.id, row.parent_name, row.phone, row.wechat, row.child_name, row.child_gender, row.child_age,
        productLabels[row.product] || row.product,
        row.parent_accompany === 'yes' ? '是' : '否', row.accompany_days, row.child_grade,
        row.has_special_needs === 'yes' ? '是' : '否', row.special_needs_detail, row.referrer, row.source,
        row.qa1, row.qa2, row.notes,
        row.early_bird ? '是' : '否', row.group_discount ? '是' : '否', row.returning_student ? '是' : '否', row.live_order ? '是' : '否',
        row.base_price, row.discount_amount, row.accompany_fee, row.total_price, row.created_at
      ].map(esc).join(','));
    }
